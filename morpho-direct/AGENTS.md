...
Locked exit path for the frozen WETH/ynETHx market: `bot/YnethxExitProxy.sol` hardcodes Morpho Blue, the market id and the owner wallet — once the owner calls Morpho's `setAuthorization(proxy, true)`, the proxy can only ever withdraw that one position to the owner. `bot/withdraw-bot.mjs` (local, per-block via WebSocket) triggers it when liquidity ≥ MIN_WETH, sends Telegram on success, self-terminates when the position is empty. Secrets in `bot/.env.bot` (gitignored, generated by `generate-wallet.mjs`); deploy via `compile-deploy.mjs` (solc-js, `--dry` to compile-check). Revoke the authorization once fully exited.
